...
class solP(FPDF):
    ...
    def solicitudesPA(request, id ):
        ...
        pdf.cell(w=0,h=5,txt="Datos de Solicitud Preaprobado por el Sistema", border='',  align='C', fill=False, ln=1)
        pdf.image('TesisApp\static\TesisApp\images\logohabib.png', x=15, y=15, w=55, h=20)
        pdf.set_font('Arial', '', 9)

        pdf.cell(w=0,h=5,txt=s.Fecha.strftime("%A, %d de %B de %Y") if hasattr(s, 'Fecha') else '', border='R', align='R', fill=False, ln=1)
        ...
        pdf.set_font('Arial', 'B', 10)
        pdf.cell(w=190,h=5,txt='Factibilidad Técnica del Proyecto:', border=0, align='L', fill=False, ln=1)
        pdf.cell(w=0,h=2,txt='', border='',  align='C', fill=False, ln=1)
        pdf.set_font('Arial', '', 10)
        pdf.cell(w=60,h=6,txt='Evaluación:', border=0, align='L', fill=False)
        pdf.cell(w=110,h=6,txt=ft.Detalle if hasattr(ft, 'Detalle') else '', border=0, align='L', fill=False, ln=1)
