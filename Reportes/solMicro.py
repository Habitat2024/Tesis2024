...
class SolicitudMicro(FPDF):
    ...
    def soliMicro(request, ids, idp):
        ...
        pdf.set_text_color(0,0,0)
        pdf.multi_cell(w=0,h=15,txt=com.ComentarioEvalEst if hasattr(com, 'ComentarioEvalEst') else '', border=1,  align='C', fill=0)
        pdf.set_text_color(255,255,255)
        pdf.multi_cell(w=0,h=5,txt='Comentario sobre la garantía ofrecida', border=1, align='C', fill=1)
        pdf.set_text_color(0,0,0)
        pdf.multi_cell(w=0,h=15,txt=com.ComentarioGaraOfr if hasattr(com, 'ComentarioGaraOfr') else '', border=1,  align='C', fill=0)
        ########
        pdf.ln(3)
        pdf.set_text_color(r,g,b)
