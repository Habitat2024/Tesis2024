...
def registrarD(request): 
    ...
    clientedg=ClienteDatoGen.objects.create(Fecha=fecha,Codigo=codigo,CalidadActu=calidadactua,NombreConzCli=nombrecc,ConocidoComo=conocidocomo,ProfesionDui=profesiondui,Nacionalidad=nacionalidad,DocumentoIden=docidentidad,NumeroDocu=numerodoc,FechaVencdoc=fechavdoc,OcupacionActiAct=ocupacionaa,DireccionDomi=direcciondomic,CorreoElec=correoe,TelefonoCelu=telcelular,TelefonoFijo=telfijo,EstatusProp=estatusp,NombreCony=nombrecony,Estado=estado,IdSolicitud=idsol)
...
def editarCliente(request, id):
    ...
    #########################################
    try:
        s=  Solicitud.objects.filter(Id=cdg.IdSolicitud.Id)
    except Solicitud.DoesNotExist:
        s=""
...
def editarD(request): 
    ...
    cdg.OcupacionActiAct=ocupacionaa
